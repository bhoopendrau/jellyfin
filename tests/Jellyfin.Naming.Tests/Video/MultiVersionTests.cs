...
namespace Jellyfin.Naming.Tests.Video
{
    public class MultiVersionTests
    {
        ...
        [Fact]
        public void Resolve_Given4KAnd1080p_Uses1080pAsPrimary()
        {
            var files = new[]
            {
                "/movies/Movie (2020)/Movie (2020) - 4k.mkv",
                "/movies/Movie (2020)/Movie (2020) - 1080p.mkv"
            };
            ...
            Assert.Single(result);
            Assert.Single(result[0].AlternateVersions);
            Assert.Contains("1080p", result[0].Files[0].Path, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("4k", result[0].AlternateVersions[0].Files[0].Path, StringComparison.OrdinalIgnoreCase);
        }
